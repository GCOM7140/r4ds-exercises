...
# the spread function can't work because there are two values of De'Andre's dob, therefore we could use "keep"
player %>% 
  group_by(name, key) %>% 
...
#'Question 2: Do you need to gather or spread preg (see below) to tidy
#'it? What variables does preg include?
preg <- tribble(
...
# It should be tidied because it has values as variable names
preg %>% 
...
# The warning message shows that the second observation has an additional piece of information (i.e. "g")
...
# The 'remove' argument determined whether to the keep or remove the original column that is parsed. It might be reasonable to keep the original column when you need it for the next similar operation
...
who %>%
  ...
  geom_point() + 
  geom_line()
#  Bangladesh in 2009, India in 2007 and China in 2008 have extreme cases that need to be further checked
...
# departure delay is the longest when the wind speed is between 30 - 35 mph
