...
# Since there are two different values of "dob" for "De' Andre Hunter", we can't tidy it up with spread() as it causes confliction.
...
# the second values "a,e,f,g" contains 4 letters which does exceeds the maximum of 3, so the elements after the first 3 will be discarded.
...
# The "remove" argument could be used to remove/keep a parsed column. The default is to remove the columns, and it is necessary to keep columns for troubleshooting.  
...
# a) low number of cases in Bangladesh in 2009
# b) consistently increasing cases in China
# c) 2007 is worth looking into in India
