...
### file name: 04_r4ds-data-wrangling-knf8ef.R
...
# The spread() function cannot be used to tidy up the date bc there isn't a unique combination of keys for each row.
...
# The warning means that the additional letter in the second obs. was dropped.
...
# The "remove" argument in separate and unite removes the input column from the output data frame. Setting "remove = FALSE" makes sense when there's a need to make further changes to the data based off that column.
...
# The period 2007-2009 stands out for all three countries as there are strng declines and pick ups in the number of TB cases.
...
# Departure delays out of NYC are the longest of average when wind speeds are at 30-35 mph.