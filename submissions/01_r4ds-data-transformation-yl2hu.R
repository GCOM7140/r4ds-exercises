...
#Missing values are always sorted at the end except when you do arrange(desc(is.na(arr_time))) 
...
flights %>% 
  filter(distance >= 2000,
         arr_delay > 0) %>%
  group_by(dest) %>%
  summarise(dest_delay = sum(arr_delay)) %>%
  mutate(total_delay = dest_delay/sum(dest_delay)) %>%
