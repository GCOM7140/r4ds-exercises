...
with the `tidyverse` package to work on them, and start by loading the 
`tidyverse`.
...
Also, run `?flights` and `?who` in the console to familiarize yourself with
these datasets.
...
**Hint:** You can use `row_number()` to create an `id` column that uniquely
identifies the observations in `people`. This `id` column will fix the problem.
...
can't figure out what it means? How could you incorporate all of the data into a 
tibble?
...
to strengthen your ability to merge relational data with 
[`inner_join()`][G&W 2017, 13.4.2] and then summarize it.
